...
def prepare_dataset():
    ...
    labeled_train_dataset = (
        tfds.load("colorectal_histology", split="train[1%:]", as_supervised=True, shuffle_files=True)#.batch(labeled_batch_size)
    )
...
def get_eval_fn(finetuning_model):
    ...
    def evaluate(weights: fl.common.Weights) -> Optional[Tuple[float, Dict]]:
        finetuning_model.set_weights(weights) 
...
strategy = fl.server.strategy.FedAvg(
    ...
    eval_fn=get_eval_fn(finetuning_model),
)

# Start Flower server for four rounds of federated learning
fl.server.start_server("localhost:3200",config={"num_rounds": 20},grpc_max_message_length=1024*1024*1024,strategy=strategy)
